...
class Audible(BeetsPlugin):
    ...
    def candidates(self, items, artist, album, va_likely, extra_tags=None):
        ...
        for a in albums:
            ...
            if is_likely_match and (not is_chapterized or not self.config['match_chapters']):
                self._log.debug(f"Attempting to match book: album {album} with {len(items)} files"
    ...
    def get_albums(self, query):
        ...
        try:
            ...
            if len(products_without_unreleased_entries) < len(products):
                ...
                self._log.info(
                    f"Excluded {len(products) - len(products_without_unreleased_entries)} books which have not been released from consideration.")
            out = []
    ...
    def fetch_art(self, session, task):
        ...
        if task.is_album:
            ...
            if task.choice_flag not in (importer.action.APPLY,
